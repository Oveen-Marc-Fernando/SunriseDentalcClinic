...
/**
 * Administration &gt; Operations &gt; Appointments — a read-only, live
 * overview of every appointment on record.
 *
 * @author oveen
 */
public class AD_OP_Appointments extends javax.swing.JFrame {
    ...
    private void installTabBar() {
        TabBarPanel tabs = new TabBarPanel(
                new String[]{"Dentists", "Patients", "Appointments", "Inventory", "Billings"}, null, 2);
    ...
    // AppointmentManagementController — Address/Contact No come along for
    // free via AppointmentDAO's LEFT JOIN to patients (city/mobileNo).
    private void populateData() {
        tableModel.setRowCount(0);
        for (model.AppointmentModel a : controller.AppointmentManagementController.getAll()) {
            tableModel.addRow(new Object[]{
                dash(a.getAppointmentId()), dash(a.getPatientName()), dash(a.getCity()), dash(a.getMobileNo()),
                dash(a.getDentistName()), dash(a.getTreatmentType()), combineDateTime(a)
            });
        }
    }

    private static String combineDateTime(model.AppointmentModel a) {
        String date = a.getDate();
        String time = a.getTime();
        boolean hasDate = date != null && !date.trim().isEmpty();
        boolean hasTime = time != null && !time.trim().isEmpty();
        if (!hasDate && !hasTime) return "-";
        if (hasDate && hasTime) return date.trim() + " / " + time.trim();
        return hasDate ? date.trim() : time.trim();
    }
    ...
    private void initComponents() {
        ...
        lblSectionTitle.setText("Appointment Details");
        ...
        tblGrid.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Appointment ID", "Patient Name", "Address", "Contact No", "Dentist Name", "Treatment Type", "Date & Time"
            }
        ) {
        ...
        if (tblGrid.getColumnModel().getColumnCount() > 0) {
            tblGrid.getColumnModel().getColumn(0).setPreferredWidth(110);
            tblGrid.getColumnModel().getColumn(1).setPreferredWidth(130);
            ...
            tblGrid.getColumnModel().getColumn(6).setPreferredWidth(140);
        }

        pnlTableWrap.add(scrollPane);
        scrollPane.setBounds(1, 1, 898, 360);

        mainPanel.add(pnlTableWrap);
        pnlTableWrap.setBounds(50, 265, 900, 360);
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> new AD_OP_Appointments().setVisible(true));
    }
