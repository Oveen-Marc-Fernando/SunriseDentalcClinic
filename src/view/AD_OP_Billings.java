...
/**
 * Administration &gt; Operations &gt; Billings — a read-only, live overview
 * of every bill on record, with a "View Receipt" link that reopens the same
 * PDF preview popup Billing's own Step 4/grid Print button uses.
 *
 * @author oveen
 */
public class AD_OP_Billings extends javax.swing.JFrame {
    ...
    private static final Color COLOR_LINK = new Color(0, 122, 255);
    private static final int VIEW_RECEIPT_COLUMN = 4;

    private DefaultTableModel tableModel;
    private TableRowSorter<DefaultTableModel> sorter;

    public AD_OP_Billings() {
        initComponents();
    ...
    private void installTabBar() {
        TabBarPanel tabs = new TabBarPanel(
                new String[]{"Dentists", "Patients", "Appointments", "Inventory", "Billings"}, null, 4);
    ...
    /** "View Receipt" column — a solid blue pill button, same chrome as OS_BM_Grid's Print button. */
    private class ViewReceiptRenderer extends DefaultTableCellRenderer {
        @Override
        public Component getTableCellRendererComponent(javax.swing.JTable t, Object v,
                boolean sel, boolean foc, int row, int col) {
            JPanel panel = new JPanel(null);
            JButton btnView = IconFactory.actionButton(null, COLOR_LINK, "View receipt");
            btnView.setText("View");
            btnView.setForeground(Color.WHITE);
            btnView.setFont(FONT_DEFAULT.deriveFont(Font.BOLD));
            btnView.setBounds((t.getColumnModel().getColumn(col).getWidth() - 78) / 2, 3, 78, 28);
            panel.add(btnView);
    ...
    private void setupTable() {
        ...
        tblGrid.getColumnModel().getColumn(VIEW_RECEIPT_COLUMN).setCellRenderer(new ViewReceiptRenderer());
        ...
        tblGrid.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override public void mouseClicked(java.awt.event.MouseEvent e) {
                ...
                if (viewRow < 0 || viewCol != VIEW_RECEIPT_COLUMN) return;
                int modelRow = tblGrid.convertRowIndexToModel(viewRow);
                String billingId = String.valueOf(tableModel.getValueAt(modelRow, 0));
                openReceipt(billingId);
            }
        });
    ...
    private void openReceipt(String billingId) {
        model.BillingModel bill = controller.BillingManagementController.getAll().stream()
                ...
                .orElse(null);
        if (bill == null) {
            javax.swing.JOptionPane.showMessageDialog(this, "Couldn't find bill " + billingId + " in the database.",
    ...
    // Email is looked up by patient_id first (reliable FK) and only falls
    // back to a name match for older billing rows that never got a
    // patient_id — patient_name can carry an honorific ("Mr. Thejaan") that
    // won't exact-match patients.full_name ("Thejaan"), so the ID lookup is
    // preferred whenever it's available.
    private void populateData() {
        tableModel.setRowCount(0);
        for (model.BillingModel b : controller.BillingManagementController.getAll()) {
            String email = controller.PatientManagementController.findEmailById(b.getPatientId());
            if (email == null || email.trim().isEmpty()) {
                email = controller.PatientManagementController.findEmailByFullName(b.getPatientName());
            }
            tableModel.addRow(new Object[]{
                dash(b.getBillingId()), dash(b.getPatientName()),
                controller.BillingManagementController.formatCurrency(b.getTotalBillAmount()),
                dash(email), "View", String.valueOf(b.getEmailSentCount())
            });
        }
    }
    ...
    private void initComponents() {
        ...
        lblSectionTitle.setText("Billing Details");
        ...
        tblGrid.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Billing ID", "Patient Name", "Total Amount", "Email", "View Receipt", "Sent Email Count"
            }
        ) {
        ...
        if (tblGrid.getColumnModel().getColumnCount() > 0) {
            ...
            tblGrid.getColumnModel().getColumn(1).setPreferredWidth(160);
            tblGrid.getColumnModel().getColumn(2).setPreferredWidth(140);
            tblGrid.getColumnModel().getColumn(3).setPreferredWidth(200);
            tblGrid.getColumnModel().getColumn(4).setPreferredWidth(110);
            tblGrid.getColumnModel().getColumn(5).setPreferredWidth(140);
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> new AD_OP_Billings().setVisible(true));
    }
