...
public class BillPreviewDialog extends JDialog {
    ...
    public BillPreviewDialog(Window owner, BillingModel bill, boolean showSuccessBanner, Runnable onClose) {
        this(owner, bill, showSuccessBanner, showSuccessBanner, onClose);
    }

    /**
     * @param owner              the screen this popup is opened from
     * @param bill               the bill to render
     * @param showSuccessBanner  true right after a bill was just saved (Save button)
     * @param showEmailButton    true to show the Email action — independent of the banner, since
     *                           reopening an already-saved bill (Print Bill / View from the grid) should
     *                           still be able to email it, not just a freshly-generated one
     * @param onClose            runs after this dialog closes — e.g. navigate back to the grid. May be null.
     */
    public BillPreviewDialog(Window owner, BillingModel bill, boolean showSuccessBanner,
        ...
        // onClose now (e.g. navigating back to the billing grid).
    ...
    private JComponent buildButtonBar(boolean showEmail) {
        ...
        // repurposed into View instead: opens the bill straight in the
     ...
     * staff can glance at the full-size bill without saving a file anywhere
     * first. The temp file is marked delete-on-exit rather than cleaned up
     * immediately, since the browser needs it to still exist after this
     * method returns.
     */
    private void viewInBrowser() {
        if (pdfBytes == null) {
            JOptionPane.showMessageDialog(this, "No bill PDF to view.", "View", JOptionPane.WARNING_MESSAGE);
            return;
        }
        try {
            File temp = File.createTempFile(
                    "Bill_" + (bill.getBillingId() != null ? bill.getBillingId() : "receipt") + "_", ".pdf");
            temp.deleteOnExit();
            ...
            JOptionPane.showMessageDialog(this, "Couldn't open the bill in your browser:\n" + e.getMessage(),
