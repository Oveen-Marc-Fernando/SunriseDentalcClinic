...
/**
 * Administration &gt; Operations &gt; Dentists — a read-only, live overview
 * of every dentist on record. Reached from the Administration Dashboard's
 * "Operations" button; the tab bar at the top switches between this and the
 * other four Operations screens without returning to the dashboard first.
 *
 * @author oveen
 */
public class AD_OP_Dentists extends javax.swing.JFrame {
    ...
    private static final int LICENSE_COLUMN = 7;

    private DefaultTableModel tableModel;
    private TableRowSorter<DefaultTableModel> sorter;

    public AD_OP_Dentists() {
        initComponents();
    ...
    private void installTabBar() {
        TabBarPanel tabs = new TabBarPanel(
                new String[]{"Dentists", "Patients", "Appointments", "Inventory", "Billings"}, null, 0);
    ...
    /** License column — small colored icon-only badge (green check / red cross), no text. */
    private class LicenseIconRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(javax.swing.JTable t, Object v,
            ...
            panel.add(IconFactory.statusDot(v != null && v.toString().equalsIgnoreCase("Active")));
            return panel;
        }
    }
    ...
    private void setupTable() {
        ...
        tblGrid.getColumnModel().getColumn(LICENSE_COLUMN).setCellRenderer(new LicenseIconRenderer());

        txtSearch.getDocument().addDocumentListener(new SearchDocumentListener());
    }

    // Backed by the real "dentists" table (db/schema.sql), via
    // DentistManagementController — the same shared directory every other
    // dentist-facing screen in the app reads from.
    ...
    private static String formatFee(String fee) {
        ...
        try {
            return String.format("%,.2f", Double.parseDouble(fee.trim()));
    ...
    private void initComponents() {
        ...
        lblSectionTitle.setText("Dentist Details");
        ...
        tblGrid.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Dentist ID", "Dentist Name", "SLMC No", "Mobile No", "Email", "Consultation Fee", "Employment Type", "License"
            }
        ) {
        ...
        if (tblGrid.getColumnModel().getColumnCount() > 0) {
            ...
            tblGrid.getColumnModel().getColumn(2).setPreferredWidth(90);
            tblGrid.getColumnModel().getColumn(3).setPreferredWidth(110);
            tblGrid.getColumnModel().getColumn(4).setPreferredWidth(150);
            tblGrid.getColumnModel().getColumn(5).setPreferredWidth(110);
            tblGrid.getColumnModel().getColumn(6).setPreferredWidth(120);
            tblGrid.getColumnModel().getColumn(7).setPreferredWidth(70);
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> new AD_OP_Dentists().setVisible(true));
    }
