...
/**
 * Step 2: Dentist &amp; Schedule Information for Appointment Management.
 *
 * Dentist Name is a dropdown of every dentist registered through Dentist
 * Management (Step 5 there is exactly "Working Days / Start Time / End Time
 * / Room No" — that's the single source of truth this screen reads from,
 * nothing is hard-coded here). Picking a dentist works out their next few
 * upcoming working dates from that text, and picking a date works out the
 * open hourly slots between their start/end time (minus break time) — both
 * calculations live in {@link AppointmentManagementController}, not here;
 * this View only renders what the controller works out. Any slot already
 * booked (checked live against the "appointments" table) is left out, so a
 * dentist can't be double-booked into it again. Appointment No is
 * auto-generated the moment this screen opens; Room No fills in as soon as
 * a dentist is picked. Both are read-only.
 *
 * The patient picked in Step 1 (OS_AM_1) is carried over via the
 * constructor and submitted along with the rest of this step's fields.
 */
public class OS_AM_2 extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        setTitle("Sunrise Dental – Appointment Management (Step 2)");
        setResizable(false);
        ...
        lblSubtitle.setText("Dentist & Schedule Information");
    ...
    private void btnSubmitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSubmitActionPerformed
        ...
        if (patientName == null) {
            JOptionPane.showMessageDialog(this,
                    "No patient selected — please start again from Step 1.",
                    "Validation Error", JOptionPane.WARNING_MESSAGE);
            return;
        }
